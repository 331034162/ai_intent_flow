...
class ZBXiaoBangRAG:
    ...
    async def process_user_input(
        ...
        app_logger.info(f"RAG工作流配置: workflow_id={workflow.workflow_id}, "
        ...
        context: ChatContext = ChatContext(
            ...
            chat_history=await AbstractAI.get_chat_history_from_db(
                conversation_id,
                message_status=1,
                max=history_max_records,
                is_human_generated=0
            ) if workflow.enhance_intent_classify == 0 and use_history else None,
            ...
            knowledge_conversation_id=knowledge_conversation_id if knowledge_conversation_id else None
        )
    ...
    async def _event_generator(
        ...
        try:
            app_logger.info(f"开始处理RAG请求: user_id={user_id}, conversation_id={conversation_id}")

            async for chunk in ZBXiaoBangRAG.process_user_input(
                ...
                # 如果状态为 done，结束生成器
                if status == "done":
                    app_logger.info(f"RAG业务处理完成 (status=done)")
                    break
                else:
    ...
    async def ai_brain_stream_endpoint(request: Request):
        ...
        - history_max_records: 历史消息最大记录数（默认5）
        - conversation_type: 会话类型（默认2）
        ...
        {"content":"文档分析完成！","status":"done","conversation_id":"abc123","content_type":"model","seq_no":"202603311200001234567"}
        """
        ...
        history_max_records = int(query_params.get('history_max_records', 5))
        conversation_type = int(query_params.get('conversation_type', 2))
        ...
        return StreamingResponse(
            ZBXiaoBangRAG._event_generator(
                user_id, user_input, conversation_id, workflow_id, seq_no,
                file_list, history_max_records, conversation_type, user_name, use_history
            ),
            media_type="text/event-stream",
...
if __name__ == "__main__":
    ...
    async def console_chat():
        """控制台对话函数"""
        print("=" * 70)
        print("欢迎使用小帮 RAG AI 助手！")
        print("=" * 70)
        print("请输入用户ID和消息与AI助手对话")
        print("-" * 70)

        # 配置参数
        workflow_id = "xiaobang_rag"  # 请根据实际情况修改为正确的RAG工作流ID
